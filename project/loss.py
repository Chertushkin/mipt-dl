...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        self.hyp = {"box": (1, 0.02, 0.2), "cls": (1, 0.2, 4.0), "obj": (1, 0.2, 4.0), "anchor_t": (1, 2.0, 8.0)}
        self.hyp = {k: torch.tensor(v) for k, v in self.hyp.items()}
    ...
    def build_targets(self, p, targets):
        ...
        for i in range(self.nl):
            ...
            if nt:
                ...
                # print('R:', r)
                # print("------")
                # print('MAXR:', torch.max(r, 1 / r).max(2)[0])
                # print("------")
                # print('MAXR:', torch.max(r, 1 / r).max(2)[0].shape)
                # print(self.hyp["anchor_t"])
                max_r = (torch.max(r, 1 / r).max(2)[0]).T
                device = "cuda"
                max_r = max_r.to(device)
                self.hyp["anchor_t"] = self.hyp["anchor_t"].to(device)
                j = max_r < self.hyp["anchor_t"]  # compare
